 ...
 * @returns 无返回值
 */
async function executeTask(db, runCnt) {
  if (runCnt >= 50) {
    log('任务执行次数已达上限，不再继续执行。')
    return
  }
  // 等待 p-limit 动态导入完成
  while (!pLimit) {
  ...
  // 筛选出符合执行条件的任务
  // const tasksToProcess = await Promise.all(
  //   result.filter(async (task) => {
  //     // 如果任务的上次执行时间不符合条件，过滤掉
  ...
  try {
    await Promise.all(
      // tasksToProcess.map((task) => processTask(db, task, browser))
      tasksToProcess.map((task) =>
        limit(() => processTask(db, task, browser, runCnt))
      )
    )
  } catch (error) {
    log('任务执行失败:', error)
 ...
 * @returns
 */
async function processTask(db, task, browser, runCnt) {
  // console.log(task)
  if (!task) {
    return
  }
  ...
  try {
    ...
    let pagea = await context.pages()
    runCnt = pagea.length
    console.log('打开的浏览器页数')
    console.log(runCnt)
...
async function list(db, row, context) {
  ...
  try {
    ...
    // 创建新的浏览器上下文并为每个任务设置代理
    // context = await browser.newContext({
    //   proxy: { server: row.proxy_url } // 设置任务特定代理
    // })
